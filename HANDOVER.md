...
- **Scanned image-only pages** (`_ocr_page` + `core/ocr.py`): some statements have NO text layer at all (e.g. the Ameris statement is 30 pages of scanned images, 0 characters). The only way to get data out is OCR. **Tesseract is now the primary OCR engine** (`core/ocr.py`), bundled into the exe as a self-contained binary (see build section below) and called via `pytesseract`. It renders the page at 300 DPI, gets word boxes from `image_to_data`, and runs them through `_grid_from_words`. Then `_trim_empty_columns` drops blank columns and `_split_leading_date` peels a leading date (06-01, 22/04/2026, etc) into its own column so Date and Description separate. Validated on the real Ameris scan on Mac (Tesseract runs on Mac too): 30 populated sheets, clean Date / Description / Additions / Subtractions columns, amounts correctly placed. Digits are accurate apart from the occasional dropped thousands-comma, so the output is a reviewed draft, not gospel (the user has a reviewer for this, which is why we shipped it).
  - **Why Tesseract, not winocr:** the user's exe showed the "needs OCR engine" note on the Ameris scan, meaning `winocr` was NOT actually running inside the frozen exe (its Windows-Runtime deps didn't survive packaging, despite `--collect-all winocr winsdk`). A bundled standalone binary we invoke is far more robust than a native-extension import. `winocr` is kept only as a last-resort fallback in `_winocr_page`.
  - **Never returns an empty page:** `_ocr_page` also returns each line's plain text; if the positional grid is empty or thin, that raw text is written instead (rough column split via `_split_line_cols`).
  - Bug fixed earlier: `_grid_from_words` sorted `(y_centre, word_dict)` tuples, raising `TypeError` when two words shared a y-centre. Now sorts by key only. Borderless-text + OCR is the only caller; the ruled path (ICICI) was unaffected.
...
11. **PyInstaller build flags currently in use** (`.github/workflows/build-windows.yml`): `--collect-all pdfminer` (PDF to Excel needs pdfminer's data files, silent failure otherwise), `--hidden-import win32timezone` (common pywin32 + PyInstaller gotcha), `--collect-all winocr --collect-all winsdk` (Custom % legibility OCR).
12. **Tesseract is bundled into the exe** for scanned PDF to Excel. The build step `choco install tesseract`, stages `tesseract.exe` + its DLLs + `eng`/`osd` traineddata into `tessbundle/`, and PyInstaller adds it with `--add-data "tessbundle;tesseract"`. At runtime `core/ocr.py` points `pytesseract.tesseract_cmd` and `TESSDATA_PREFIX` at `<_MEIPASS>/tesseract/`. From source (dev/Mac), it falls back to a system Tesseract on PATH (`brew install tesseract`). This is the one part not verifiable from Mac: confirm the built exe actually finds Tesseract (convert a scanned PDF and check it isn't the "could not be reached" note). If it can't find it, check the bundled folder name matches what `_configure()` looks for (`tesseract/tesseract.exe`, `tesseract/tessdata`).
